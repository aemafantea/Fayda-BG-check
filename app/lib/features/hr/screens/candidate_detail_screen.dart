  ...
  Future<Map<String, dynamic>?> _loadAll() async {
    ...
    try {
      ...
      final docs = await sb
          .from('documents')
          .select()
          .eq('owner_id', cand['profile_id']);
      return {'cand': cand, 'emp': emp, 'checks': checks, 'docs': docs};
    } catch (e) {
      // schema-missing or other → null
      return null;
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: RefreshIndicator(
        onRefresh: () async => setState(() => _future = _loadAll()),
            ...
            return ListView(
              ...
              children: [
                Card(
                  child: ListTile(
                    ...
                    trailing: StatusBadge.fromStatus(
                        prof['fayda_verification_status']?.toString()),
                  ),
                ),
                ...
                FilledButton.icon(
                  ...
                  onPressed: () =>
                      context.go('/hr/new-check?candidate=${widget.candidateId}'),
                  label: const Text('Start new background check'),
                ),
                const SizedBox(height: 16),
                const Text('Background checks',
                    style:
                ...
                if (checks.isEmpty)
                  const Card(
                      child:
                          ListTile(title: Text('No background checks yet'))),
                ...checks.map((b) => Card(
                      child: ListTile(
                        title: Text(
                            'Check #${(b['id'] as String).substring(0, 8)}'),
                        subtitle: Text(
                            'Created ${DateTime.parse(b['created_at']).toLocal()}'),
                        trailing: Row(
                          mainAxisSize: MainAxisSize.min,
                          children: [
                            StatusBadge.fromStatus(b['status']?.toString()),
                            const SizedBox(width: 6),
                            StatusBadge.fromRisk(b['risk_level']?.toString()),
                          ],
                        ),
                ...
                const SizedBox(height: 16),
                Text('Employment history (${emp.length})',
                    style: const TextStyle(
                        fontWeight: FontWeight.w700, fontSize: 16)),
                const SizedBox(height: 6),
                if (emp.isEmpty)
                  const Card(
                      child: ListTile(title: Text('No employment recorded'))),
                ...emp.map((e) => Card(
                      child: ListTile(
                        title: Text(
                            '${e['position_title']} @ ${e['employer_name']}'),
                        subtitle: Text(
                            '${fmt.format(DateTime.parse(e['start_date']))} → '
                            '${e['end_date'] != null ? fmt.format(DateTime.parse(e['end_date'])) : 'present'}'),
                        trailing: e['verified'] == true
                            ? const StatusBadge(
                                label: 'verified',
                                color: AppTheme.success)
                            : TextButton(
                                onPressed: () => _verifyJob(e['id']),
                                child: const Text('Verify')),
                      ),
                    )),
                const SizedBox(height: 16),
                ...
                if (docs.isEmpty)
                  const Card(
                      child: ListTile(title: Text('No documents uploaded'))),
                ...docs.map((d) => Card(
                      child: ListTile(
